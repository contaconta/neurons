...
/*
	...
	trees from one maxflow computation to the next, as described in
		...
		"Efficiently Solving Dynamic Markov Random Fields Using DynamicGraph Cuts."
...
template <typename captype, typename tcaptype, typename flowtype> class DynamicGraph
{
public:
	typedef enum
	{
	...
	DynamicGraph(int node_num_max, int edge_num_max, void (*err_function)(char *) = NULL);
	...
	// segment the node 'i' belongs (DynamicGraph<captype,tcaptype,flowtype>::SOURCE or DynamicGraph<captype,tcaptype,flowtype>::SINK).
	//
...
public:
	...
	//
	// Advantage compared to deleting DynamicGraph and allocating it again:
	...
	tcaptype MIN_TCAP(tcaptype a, tcaptype b);
	tcaptype MAX_TCAP(tcaptype a, tcaptype b);
	...
	//
	// NOTE: The user should not deallocate the returned list. (This will be done by DynamicGraph destructor).
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename DynamicGraph<captype,tcaptype,flowtype>::node_id DynamicGraph<captype,tcaptype,flowtype>::add_node(int num)
{
	assert(num > 0);
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void DynamicGraph<captype,tcaptype,flowtype>::add_edge(node_id _i, node_id _j, captype cap, captype rev_cap)
{
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename DynamicGraph<captype,tcaptype,flowtype>::arc* DynamicGraph<captype,tcaptype,flowtype>::get_first_arc()
{
	return arcs;
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline typename DynamicGraph<captype,tcaptype,flowtype>::arc* DynamicGraph<captype,tcaptype,flowtype>::get_next_arc(arc* a) 
{
	return a + 1; 
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline void DynamicGraph<captype,tcaptype,flowtype>::get_arc_ends(arc* a, node_id& i, node_id& j)
{
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline tcaptype DynamicGraph<captype,tcaptype,flowtype>::get_trcap(node_id i)
{
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline captype DynamicGraph<captype,tcaptype,flowtype>::get_rcap(arc* a)
{
	assert(a >= arcs && a < arc_last);
	return a->r_cap;
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void DynamicGraph<captype,tcaptype,flowtype>::set_trcap(node_id i, tcaptype trcap)
{
	assert(i>=0 && i<node_num); 
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void DynamicGraph<captype,tcaptype,flowtype>::set_rcap(arc* a, captype rcap)
{
	assert(a >= arcs && a < arc_last);
	a->r_cap = rcap;
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename DynamicGraph<captype,tcaptype,flowtype>::termtype DynamicGraph<captype,tcaptype,flowtype>::what_segment(node_id i, termtype default_segm)
{
	if (nodes[i].parent)
	{
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void DynamicGraph<captype,tcaptype,flowtype>::mark_node(node_id _i)
{
	node* i = nodes + _i;
	if (!i->next)
	{
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline tcaptype DynamicGraph<captype,tcaptype,flowtype>::MIN_TCAP(tcaptype a, tcaptype b)
{
	if (a<b) return a;
	return b;
}
...
template <typename captype, typename tcaptype, typename flowtype> 
inline tcaptype DynamicGraph<captype,tcaptype,flowtype>::MAX_TCAP(tcaptype a, tcaptype b)
{
	if (a>b) return a;
	return b;
}
