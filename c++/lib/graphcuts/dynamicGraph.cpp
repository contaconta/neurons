...
template class DynamicGraph<int,int,int>;
template class DynamicGraph<short,int,int>;
template class DynamicGraph<float,float,float>;
template class DynamicGraph<double,double,double>;
...
template <typename captype, typename tcaptype, typename flowtype> 
	DynamicGraph<captype, tcaptype, flowtype>::DynamicGraph(int node_num_max, int edge_num_max, void (*err_function)(char *))
	: node_num(0),
	  nodeptr_block(NULL),
	  changed_list(NULL),
...
template <typename captype, typename tcaptype, typename flowtype> 
	DynamicGraph<captype,tcaptype,flowtype>::~DynamicGraph()
{
	if (nodeptr_block) 
	{ 
...
template <typename captype, typename tcaptype, typename flowtype> 
	void DynamicGraph<captype,tcaptype,flowtype>::reset()
{
	node_last = nodes;
	arc_last = arcs;
	node_num = 0;
...
template <typename captype, typename tcaptype, typename flowtype> 
	void DynamicGraph<captype,tcaptype,flowtype>::reallocate_nodes(int num)
{
...
template <typename captype, typename tcaptype, typename flowtype> 
	void DynamicGraph<captype,tcaptype,flowtype>::reallocate_arcs()
{
...
template <typename captype, typename tcaptype, typename flowtype> 
void DynamicGraph<captype,tcaptype,flowtype>::edit_tweights(node_id i, tcaptype cap_source, tcaptype cap_sink)
{
	tcaptype oldRes = nodes[i].t_cap;

	if (nodes[i].t_cap != cap_source - cap_sink) 
	{
		if (nodes[i].t_cap>0) flow -= MIN_TCAP(nodes[i].t_cap-nodes[i].tr_cap,nodes[i].t_cap);
		else flow += MAX_TCAP(0,nodes[i].tr_cap);
	...
	nodes[i].con_flow = MIN_TCAP(cap_source, cap_sink);
	flow += MIN_TCAP(cap_source, cap_sink);
}	

template <typename captype, typename tcaptype, typename flowtype> 
void DynamicGraph<captype,tcaptype,flowtype>::edit_tweights_wt(node_id i, tcaptype cap_source, tcaptype cap_sink)
{
	tcaptype oldRes = nodes[i].t_cap;
...
template <typename captype, typename tcaptype, typename flowtype> 
void DynamicGraph<captype,tcaptype,flowtype>::edit_edge(node_id from, node_id to, captype cap, captype rev_cap)
{
	arc *a, *a_rev;
	a = nodes[from].first;
	...
	else
	{
		...
		if (nodes[from].t_cap>0) flow -= MIN_TCAP(nodes[from].t_cap-nodes[from].tr_cap,nodes[from].t_cap);
		else flow += MAX_TCAP(0,nodes[from].tr_cap);

		if (nodes[to].t_cap>0) flow -= MIN_TCAP(nodes[to].t_cap-nodes[to].tr_cap,nodes[to].t_cap);
		else flow += MAX_TCAP(0,nodes[to].tr_cap);
...
template <typename captype, typename tcaptype, typename flowtype> 
void DynamicGraph<captype,tcaptype,flowtype>::edit_edge_wt(node_id from, node_id to, captype cap, captype rev_cap)
{
	arc *a, *a_rev;
	a = nodes[from].first;
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void DynamicGraph<captype,tcaptype,flowtype>::set_active(node *i)
{
	if (!i->next)
	{
...
*/
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename DynamicGraph<captype,tcaptype,flowtype>::node* DynamicGraph<captype,tcaptype,flowtype>::next_active()
{
	node *i;
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void DynamicGraph<captype,tcaptype,flowtype>::set_orphan_front(node *i)
{
	nodeptr *np;
	i -> parent = ORPHAN;
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void DynamicGraph<captype,tcaptype,flowtype>::set_orphan_rear(node *i)
{
	nodeptr *np;
	i -> parent = ORPHAN;
	np = nodeptr_block -> New();
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void DynamicGraph<captype,tcaptype,flowtype>::add_to_changed_list(node *i)
{
...
template <typename captype, typename tcaptype, typename flowtype> 
	void DynamicGraph<captype,tcaptype,flowtype>::maxflow_init()
{
	node *i;
...
template <typename captype, typename tcaptype, typename flowtype> 
	void DynamicGraph<captype,tcaptype,flowtype>::maxflow_reuse_trees_init()
{
	node* i;
	node* j;
...
template <typename captype, typename tcaptype, typename flowtype> 
	void DynamicGraph<captype,tcaptype,flowtype>::augment(arc *middle_arc)
{
	node *i;
	arc *a;
	tcaptype bottleneck;
...
template <typename captype, typename tcaptype, typename flowtype> 
	void DynamicGraph<captype,tcaptype,flowtype>::process_source_orphan(node *i)
{
	node *j;
...
template <typename captype, typename tcaptype, typename flowtype> 
	void DynamicGraph<captype,tcaptype,flowtype>::process_sink_orphan(node *i)
{
	node *j;
	arc *a0, *a0_min = NULL, *a;
	int d, d_min = INFINITE_D;
...
template <typename captype, typename tcaptype, typename flowtype> 
	flowtype DynamicGraph<captype,tcaptype,flowtype>::maxflow(bool reuse_trees, Block<node_id>** _changed_list)
{
...
template <typename captype, typename tcaptype, typename flowtype> 
	void DynamicGraph<captype,tcaptype,flowtype>::test_consistency(node* current_node)
{
	node *i;
	arc *a;
	int r;
