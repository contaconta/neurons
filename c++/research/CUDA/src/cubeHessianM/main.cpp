  ...
  )
{
  ...
  putGPU(d_Buffer, -2, sizeX, sizeY, sizeZ);

  for(float sigma = sigma_b; sigma <= sigma_e; sigma+=sigma_s)
    {
      ...
      hessianGPU(d_tmp, d_gxx, d_gxy, d_gxy, d_gyy, d_gyz, d_gzz,
                 sigma, sizeX, sizeY, sizeZ);

      maxGPU    (d_Buffer, d_tmp, sizeX, sizeY, sizeZ);
    }
...
int main(int argc, char **argv){
  ...
  if(argc!=6){
    printf("Usage: cubeHessian volume.nfo sigma_b sigma_e sigma_s out\n");
    exit(0);
  }
  ...
  float sigma_b = atof(argv[2]);
  float sigma_e = atof(argv[3]);
  float sigma_s = atof(argv[4]);
  Cube<float, double>* res = cube->create_blank_cube(argv[5]);
  ...
  cutilSafeCall( cudaMalloc((void **)&d_tmp,   maxLinearSize *sizeof(float)) );

  // Here should come the loop
  // Variables required to split the image into tiles
  vector< float > kernelSample = Mask::gaussian_mask(0, sigma_e, 1);
  ...
  printf("Sigma = %f, kernel size: %i, Tiles pads be: [%i,%i,%i]\n",
         sigma_e, kernelSample.size(), pad_x, pad_y, pad_z);
  ...
  // for(int tz = 0; tz < 1; tz++){
    for(int ty = 0; ty < n_tiles_vert; ty++){
      for(int tx = 0; tx < n_tiles_horiz; tx++){
        ...
        hessian
          (d_Output,
           d_Input, sigma_b, sigma_e, sigma_s,
           d_gxx, d_gxy, d_gxz, d_gyy, d_gyz, d_gzz,
           d_tmp,
           maxTileSizeX, maxTileSizeY, maxTileSizeZ);
