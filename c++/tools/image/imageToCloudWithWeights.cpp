...
vector< double > getCumulativePdf(Image< float >* img)
{
  //First we will check that all the values of the image have a positive value.
  // if not, we will add the minimum value to all the points.
  double min_val = 1e8;
  for(int y = 0; y < img->height; y++)
    for(int x = 0; x < img->width; x++)
      if(img->at(x,y) < min_val)
        min_val = img->at(x,y);
...
int findClosestIdxFromOrderedVector(vector< double > &vct, double val)
{
  //Let's do it slow, as it wont be done many times (once per point)
  int idx = 0;
  double min_diff = 1e24;
  for(int i = 0; i < vct.size(); i++){
    if( fabs(val-vct[i]) < min_diff) {
      idx = i;
      min_diff = fabs(val-vct[i]);
    }
  }
  return idx;
}

int sampleCumulativePdf(vector< double > &cpdf)
{
  // double val = gsl_rng_uniform(r)*cpdf[cpdf.size()-1];
  double val = gsl_rng_uniform(r);
  int idx = findClosestIdxFromOrderedVector(cpdf, val);
  return idx;
}
...
int main(int argc, char **argv) {
  ...
  vector< double > cumPDF = getCumulativePdf(img);
  ...
  while(nPoints < args.number_negative_points){
    ...
    int idx = sampleCumulativePdf(cumPDF);
