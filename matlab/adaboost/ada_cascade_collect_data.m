...
LEARNERS = varargin{3};  
learner_list = {LEARNERS(:).feature_type};
...
if INIT
    ...
    for i = 1:length(d)
        if count <= POS_LIM
            ...
            if ismember('haar', learner_list); II = integral_image(I); end
            if ismember('spedge', learner_list); sp = spedges(I, angles, sigma);end

            P(count).Image = I; %#ok<AGROW>
            if ismember('haar', learner_list); P(count).II    = II(:); end
            if ismember('spedge', learner_list); P(count).sp = sp.spedges(:); end
            P(count).class = 1; 
            count = count + 1;
        end
    end
...
while length(N) < NEG_LIM
    ...
    if INIT
        ...
        if ismember('haar', learner_list); N(length(N)).II    = II(:); end%#ok<AGROW>
        if ismember('spedge', learner_list); N(length(N)).sp = sp.spedges(:); end
