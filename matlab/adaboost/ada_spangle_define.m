function SP = ada_spangle_define(varargin)
%
    ...
    for j = 1:ceil(IMSIZE(2)/stride);
        for k = 1:length(angles);
            for s = 1:length(edge_methods);
                ...
                % because we will flatten the spangle tensor into a vector 
                % later, we need to order the spangle features in the order 
