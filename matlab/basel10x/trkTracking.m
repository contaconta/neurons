function G =  trkTracking(folder, resultsFolder, SeqIndexStr, Sample, params)
...
tic;
h1 = fspecial('log',[5*sigma_log_red 5*sigma_log_red], sigma_log_red);    % Laplacian filter kernel used to find nuclei
parfor  t1 = 1:TMAX
    Rt = mat2gray(double(R{t1}));
    Rblur{t1} = imgaussian(Rt,sigma_red);
...
mv = trkRenderImagesNucleus(TMAX, G, date_txt, num_txt, label_txt, cols, mv, Dlist, tracks, D, DISPLAY_FIGURES, SHOW_FALSE_DETECTS);
%%
