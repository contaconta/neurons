...
for i = 1:length(listOfGTSeq)
    if(listOfGTSeq(i).isdir && ~isempty(str2num(listOfGTSeq(i).name)) ) %#ok
        ...
        for k =1:size(PositivePairsPerSeq{seqIdx}, 1)
            ...
            hist1 = PositivePairsPerSeq{seqIdx}(k, 1).SomaHistGreen / sum(PositivePairsPerSeq{seqIdx}(k, 1).SomaHistGreen);
            hist2 = PositivePairsPerSeq{seqIdx}(k, 2).SomaHistGreen / sum(PositivePairsPerSeq{seqIdx}(k, 2).SomaHistGreen);
            PositiveEMDs(k) = emd_hat_gd_metric_mex(hist1,...
                                                    hist2,...
                                                    punishMatrix,-1);
        ...
        for k =1:size(NegativePairsPerSeq{seqIdx}, 1)
            ...
            hist1 = NegativePairsPerSeq{seqIdx}(k, 1).SomaHistGreen / sum(NegativePairsPerSeq{seqIdx}(k, 1).SomaHistGreen);
            hist2 = NegativePairsPerSeq{seqIdx}(k, 2).SomaHistGreen / sum(NegativePairsPerSeq{seqIdx}(k, 2).SomaHistGreen);
            NegativeEMDs(k) = emd_hat_gd_metric_mex(hist1,...
                                                    hist2,...
                                                    punishMatrix,-1);
        end
