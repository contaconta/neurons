function ORIENTATION = orientationEstimate2(I,S)

MINIMUM_S = 3;

%S = denseScaleEstimate(I);
ORIENTATION = zeros(size(S));

smax = max(S(:));

norms = zeros([size(S) smax]);
orients = zeros(size(norms));

for s = 1:smax
    
    Sused = max(MINIMUM_S, s);
    
    [GN, OR] = gradientEstimate(I, Sused);
    
    norms(:,:,s) = GN;
    orients(:,:,s) = OR;
    
    ORIENTATION(S == s) = OR(S == s);
