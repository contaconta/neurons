resultname = 'heathrowRayHist4class';
...
%     N0 = round(.33*N);
    ...
    for i = trainImgs
        ...
        cls1 = find(labels == 1);
        negB = find(labels == 0);
        negBH = find(   (labels == 0) & (featureVector(:,1) > INTENSITY_THRESH) );  NBH = round(.5*N0);
        negBL = find(   (labels == 0) & (featureVector(:,1) <= INTENSITY_THRESH) ); NBL = N0 - NBH;
...
%     model = svmtrain(TRAIN_L, TRAIN, cmd);
    ...
    for i = testImgs
        ...
        STATS = regionprops(L, 'PixelIdxlist', 'Centroid', 'Area');
        mito_label_index = find(model.Label == 2); P2 = zeros(size(I)); P1 = P2; P0=P1; P3=P1; Pv=P1; 
