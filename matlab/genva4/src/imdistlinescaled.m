%imdistlinescaled Draggable Distance tool.
%   H = imdistlinescaled creates a draggable distance tool on the current
%   axes. The function returns H, a handle to an imdistlinescaled object.
%
%   H = imdistlinescaled(HPARENT) creates a draggable distance tool on the object
%   specified by HPARENT. HPARENT specifies the HG parent of the imdistlinescaled
%   graphics, which is typically an axes but can also be any other object
%   that can be the parent of an hggroup.
%
%   H = imdistlinescaled(...,X,Y) creates a draggable distance tool with endpoints
...
%   Remarks 
%   ------- 
%   If you use imdistlinescaled with an axis that contains an image
...
%
%   To understand how imdistlinescaled calculates the angle returned by
...
%   h1 = imdistlinescaled(gca,[271 471],[108 650]);
...
%    
%   % Specify initial position of distance tool on Harvard Bridge.
%   h2 = imdistlinescaled(gca,[271 471],[108 650]);
%   setLabelTextFormatter(h2,'%02.0f meters');        
%       
%   See also makeConstrainToRectFcn.

%   Copyright 2005-2009 The MathWorks, Inc.
%   $Revision: 1.1.6.16 $  $Date: 2009/04/15 23:12:01 $

classdef imdistlinescaled < imline
    ...
    methods
        ...
        function obj = imdistlinescaled(varargin)
            %imdistlinescaled  Constructor for imdistlinescaled.
        ...
        function distance = getDistance(obj)
            ...
            pos = obj.getPosition();
            s = obj.getScale();
            distance = s(1) * norm([diff(pos(:,1)), diff(pos(:,2))]);
        ...
        function setScale(obj,scale_in)
            %setLabelTextFormatter  Set format string used in displaying
            %distance label.
            %
            %   setLabelTextFormatter(h,str) sets the format string used in
        ...
        function setLabelVisible(obj,val)
            ...
            if islogical(val) || isnumeric(val)
                ...
                error('Images:imdistlinescaled:setLabelVisible:invalidInput', ...
                    'setLabelVisible expects either true or false.');
            end
    ...
    methods (Access = 'private')
        ...
        function createContextMenu(obj)
            ...
            cmenu = uicontextmenu('Parent', fig, ...
                'Tag','imdistlinescaled context menu');
            ...
            % Reparent original set color context menu to imdistlinescaled context menu.
            ...
            constrain_drag_menu = uimenu(cmenu,...
                ...
                'Label','Constrain Drag');

            horiz_menu = uimenu(constrain_drag_menu,...
                'Label','Constrain to Horizontal',...
        ...
        function exportToWorkspace(obj)
            labels = {'Point 1', 'Point 2', 'Distance'};
...
if ~ishghandle(h_parent)
    error('Images:imdistlinescaled:invalidHandle', ...
        'HPARENT must be a valid graphics handle.');
end

if (nargin == 2)
    error('Images:imdistlinescaled:invalidPosition', ...
        'End point positions must be specified as X and Y position vectors.');
end

if (nargin > 3)
    error('Images:imdistlinescaled:invalidSyntax',...
          'Too many input arguments specified.');
end

invalid_three_arg_syntax = (nargin == 3) &&...
                (~isnumeric(varargin{2}) || ~isnumeric(varargin{3})); 
if invalid_three_arg_syntax
    error('Images:imdistlinescaled:invalidType',...
          'Second and third input arguments must be numeric X/Y vectors.');
end

h_axes = ancestor(h_parent,'axes');
                    
if nargin == 3
    iptcheckinput(varargin{2},{'numeric'},{'vector','real','nonempty','nonnan'},'imdistlinescaled','X',2);
    iptcheckinput(varargin{3},{'numeric'},{'vector','real','nonempty','nonnan'},'imdistlinescaled','Y',3);
    x_pos = varargin{2};
    y_pos = varargin{3};
else
