function [ROWS, COLS, INDS, POLS] = generate_viola_jones_features_special(varargin)
...
if ismember('vert3', SHAPES)
    MIN_W = max(4,quant(U_MIN_W,4));            	% min feature width
    MIN_H = U_MIN_H;                                % min feature height
    W_STEP = max(4,quant(U_STEP_W,4));              % feature width size step
    H_STEP = U_STEP_H;                              % feature height size step
        ...
        for h = MIN_H:H_STEP:MAX_H        
            for r = 1:SCAN_R_STEP:IMAGE_H - h + 1
                for c = 1:SCAN_C_STEP:IMAGE_W - w + 1
                    ...
                    C1 = [c c+w/4 c c+w/4];
                    R2 = [r r r+h r+h];
                    C2 = [c+w/4 c+(3/4)*w c+w/4 c+(3/4)*w];
                    R3 = [r r r+h r+h];
                    C3 = [c+(3/4)*w c+w c+(3/4)*w c+w];
...
if ismember('horz3', SHAPES)
    MIN_W = U_MIN_W;                                % min feature width
    MIN_H = max(4,quant(U_MIN_H,4));                % min feature height
    W_STEP = U_STEP_W;                              % feature width size step
    H_STEP = max(4,quant(U_STEP_H,4));              % feature height size step
        ...
        for h = MIN_H:H_STEP:MAX_H        
            for r = 1:SCAN_R_STEP:IMAGE_H - h + 1
                for c = 1:SCAN_C_STEP:IMAGE_W - w + 1
                    ...
                    R1 = [r r r+h/4 r+h/4];
                    C1 = [c c+w c c+w];
                    R2 = [r+h/4 r+h/4 r+(3/4)*h r+(3/4)*h];
                    C2 = [c c+w c c+w];
                    R3 = [r+(3/4)*h r+(3/4)*h r+h r+h];
                    C3 = [c c+w c c+w];
