...
%----------
train_settings;
path(path, [datapath '/violajones/']);          % include vj_functions are in the path
versioninfo;
...
logfile(log_filenm, 'column_labels', {'stage', 'step', 'Weak ID', 'pol', 'theta', 'Di', 'Fi', 'di', 'fi', 'di(train)', 'fi(train)'});

% load the nucleus locations in the MIP images so we can recollect FP data
load([update0 'NUCLEUS_LOCATIONS.mat']);
...
while (Fi > Ftarget)
    ...
    TRAIN = nucleus_cascade_collect_data(train1, update0, TRAIN, CASCADE, 'size', IMSIZE, ...
                    'normalize', NORM, 'data_limit', [TRAIN_POS TRAIN_NEG], 'nuclei_locs', NUCLEUS_LOCATIONS);
    
    
    disp('...updating the VALIDATION set with negative examples which cause false positives');
    VALIDATION = nucleus_cascade_collect_data(validation1, update0, VALIDATION, CASCADE, 'size', IMSIZE, ...
                   'normalize', NORM, 'data_limit', [TEST_POS TEST_NEG], 'nuclei_locs', NUCLEUS_LOCATIONS);
