function rvc = ffMeanGreenIntensitySomata(R, varargin)
...
for d = 1:length(R.D)
   ...
   if(isempty(R.Soma(d).MeanGreenIntensity))
     disp('encountered an empty value in ffMeanGreenIntensitySomata');
     keyboard;
   end
   if(R.Soma(d).Time == 1)
     continue;
   ...
   nPoints = nPoints+1;
   rvc(nPoints) = R.Soma(d).MeanGreenIntensity;
end
