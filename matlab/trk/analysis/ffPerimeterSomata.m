function rvc = ffPerimeterSomata(R, varargin)
...
for d = 1:length(R.D)
   ...
   if(isempty(R.Soma(d).Perimeter))
     disp('encountered an empty value in ffPerimeterSomata');
     keyboard;
   end
   if(R.Soma(d).Time == 1)
     continue;
   ...
   nPoints = nPoints+1;
   rvc(nPoints) = R.Soma(d).Perimeter;
end
