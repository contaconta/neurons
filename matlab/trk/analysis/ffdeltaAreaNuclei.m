function rvc = ffdeltadeltaAreaNuclei(R, varargin)
...
for d = 1:length(R.D)
   ...
   if(isempty(R.D(d).deltaArea))
     disp('encountered an empty value in ffdeltadeltaAreaNuclei');
     keyboard; 
   end
   nPoints = nPoints+1;

   rvc(nPoints) = R.D(d).deltaArea;
end
