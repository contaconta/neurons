function rvc = ffdeltadeltaAreaSomata(R, varargin)
...
for d = 1:length(R.D)
   ...
   if(isempty(R.Soma(d).deltaArea))
     disp('encountered an empty value in ffdeltadeltaAreaSomata');
     keyboard;
   end
   if(R.Soma(d).Time == 1)
     continue;
   ...
   nPoints = nPoints+1;
   rvc(nPoints) = R.Soma(d).deltaArea;
end
