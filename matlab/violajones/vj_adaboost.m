function CLASSIFIER = vj_adaboost(varargin)
%% VJ_ADABOOST trains a strong classifier from weak classifiers & training data.
%
%   CLASSIFIER = vj_adaboost(PRE, TRAIN, WEAK, T) trains a strong
...
%   calling CLASSIFIER = vj_adaboost(PRE, TRAIN, WEAK, T, CLASSIFIER);.
%
%
%   Copyright © 2008 Kevin Smith
%   See also VJ_TRAIN, VJ_COLLECT_DATA, VJ_DEFINE_CLASSIFIERS, BIGMATRIX_GET_ROW
...
if nargin == 4
    ...
    CLASSIFIER = vj_classifier_init(T, WEAK);
    tmin = 1;
else
...
for t = tmin:T
    ...
    for i = 1:(size(WEAK.descriptor,1))
        %[WEAK, PRE] = vj_find_haar_parameters(i, TRAIN, PRE, WEAK, w);
    ...
    f = vj_fast_haar_response(CLASSIFIER.fast(t,:), IIs);
