...
%logfile(EXPERIMENT.log_filenm, 'erase');
logfile(EXPERIMENT.log_filenm, 'header', {INFO.appname, ['Version ' INFO.version], ['by ' INFO.author ', ' INFO.email], [num2str(DATASETS.TRAIN_POS) ' positive examples, ' num2str(DATASETS.TRAIN_NEG) ' negative examples.'], ['DATASETS from ' DATASETS.pos_query], ['LEARNERS ' strcat(LEARNERS.types{:})],['Started at ' datestr(now)], INFO.copyright, '-----------------------------------'});
...
[TRAIN, DATASETS] = p_collect_data(DATASETS, LEARNERS, 'train'); %disp(['   Elapsed time ' num2str(toc) ' seconds.']); TRAIN.precomputed = 1;

% precompute feature responses on TRAIN set
...
%[VALIDATION, DATASETS] = p_collect_data2(DATASETS, 'validation'); disp(['   Elapsed time ' num2str(toc) ' seconds.']); VALIDATION.precomputed = 0;
[VALIDATION, DATASETS] = p_collect_data(DATASETS, LEARNERS, 'validation'); %disp(['   Elapsed time ' num2str(toc) ' seconds.']); VALIDATION.precomputed = 0;
...
while (Fi > BOOST.targetF)                  % create new cascade stages until we meet target false positive rate
    ...
    [TRAIN, DATASETS] = p_collect_data(DATASETS, LEARNERS, 'update', TRAIN, CASCADE);
    disp('    ---updating the VALIDATION set with negative examples which cause false positives---');
    [VALIDATION, DATASETS] = p_collect_data(DATASETS, LEARNERS, 'update', VALIDATION, CASCADE);
    if TRAIN.precomputed; p_precompute_features(TRAIN, LEARNERS); end
