...
template <class T, class U>
void Cube<T,U>::draw(float rotx, float roty, float nPlanes, int min_max, int microm_voxels)
{
  ...
  if( (x0!=x0_old) || (x1!=x1_old) ||
      ...
      (threshold_old!=threshold))
    {
      ...
      if(sizeof(T)==1){
        ...
        float x_step = max(1.0,(double)fabs((float)x1-(float)x0)/512);
        float y_step = max(1.0,(double)fabs((float)y1-(float)y0)/512);
