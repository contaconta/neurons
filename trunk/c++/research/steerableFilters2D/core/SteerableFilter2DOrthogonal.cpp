...
SteerableFilter2DOrthogonal::SteerableFilter2DOrthogonal
...
SteerableFilter2DOrthogonal::SteerableFilter2DOrthogonal(string filename_image, string filename_parameters, float sigma)
{
...
void SteerableFilter2DOrthogonal::calculate_coefficients()
{
  ifstream inp;
  char buff[512];
  sprintf(buff, "_%02.02f.jpg",sigma);
  string name_b = directory + "go" + buff;

  int k_idx = 0;
  for(int k = 1; k <= M; k++){
    for(int j = 0; j <= k; j++){
      string name = directory + "go_";
      for(int l = 0; l < k-j; l++)
        name = name + "x";
      for(int l = 0; l < j; l++)
        name = name + "y";
      ...
      if(inp.fail()){
        derivatives[k_idx + j] =
          image->calculate_gaussian_orthogonal(k-j,j,sigma, name);
      }else{
...
void SteerableFilter2DOrthogonal::calculate_steering_coefficients(double theta)
{
  double ct = cos(theta);
  double st = sin(theta);
    ...
    for(int j = 0; j <= k; j++){
      ...
      for(int i = 0; i <= k; i++){
        ...
        b_kj += gsl_vector_get(alpha,k_idx + i)*sum/
          Mask::energy2DGaussianMaskOrthogonal(k-i,i,sigma);
      }
      b_kj *= Mask::energy2DGaussianMaskOrthogonal(k-j,j,sigma);
