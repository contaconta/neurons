...
int main(int argc, char **argv) {
  ...
  if(argc!=5){
    printf("Usage: imageLogistic imageFromWhichGetTheValues cloudWithPoints OriginalImage SigmoidImage\n");
    exit(0);
  }
  string imageDataName(argv[1]);
  string cloudName   (argv[2]);
  string imageName    (argv[3]);
  string outputName  (argv[4]);

  param  = new Image<float>(imageDataName);
//     ("/media/neurons/steerableFilters3D/resultVolumes/result-4-e-2-2-groundTruth-1-estimated.nfo");

  cl = new Cloud<Point2Dot>(cloudName);
  clp = new Cloud<Point3Dot>("/scratch/ggonzale/drive/d22/22_training.cl");
...
//     ("/home/ggonzale/mount/cvlabfiler/n7_4/cut.nfo");
  ...
  Image<float>* flts = orig->create_blank_image_float(outputName);
//     ("cuts");
...
//   xv.resize(5000);
  ...
  if(0){
    for(int i = 0; i<clp->points.size(); i++){
    //for(int i = 0; i<1250; i++){
      Point3Dot* pp = dynamic_cast<Point3Dot*>(clp->points[i]);
      if(pp->type == 1){
        t.push_back(1);
        nPp++;
        mic[0] = clp->points[i]->coords[0];
        mic[1] = clp->points[i]->coords[1];
        mic[2] = 0;
        param->micrometersToIndexes(mic, idx);
        xv.push_back(param->at(idx[0], idx[1]));
      }
    }
  }
...
//   for(int i = 0; i<5000; i++){
    Point2Dot* pp = dynamic_cast<Point2Dot*>(cl->points[i]);
    if(pp->type == -1){
      t.push_back(1);
      nPp++;
