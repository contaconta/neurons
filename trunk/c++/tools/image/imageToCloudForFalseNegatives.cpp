...
const char *argp_program_version =
  "imageToCloudForFalseNegatives 0.1";
const char *argp_program_bug_address =
  "<german.gonzalez@epfl.ch>";
/* Program documentation. */
static char doc[] =
  "imageToCloudForFalseNegatives sample the image according to the weights of the points that are lower to 0 ... blah blah blah";
...
static struct argp_option options[] = {
  ...
  {"mask",          'm',  "mask_image",        0, "mask where the ground truth is"},
  {"mask_ignore",   'i',  "mask_image",        0, "if true, do not take any point whose mask is 0"},
...
vector< double > getCumulativePdf(Image< float >* img, Image< float >* mask, Image< float>* mask_ignore)
{
  ...
  for(int y = 0; y < img->height; y++){
    for(int x = 0; x < img->width; x++){
      //If it is a negative point and there is a mask and the mask is saying it is good
      if((img->at(x,y) < 0) && (mask!= NULL) && (mask->at(x,y) < 100) && (mask_ignore->at(x,y) > 100) )
        cum = cum - img->at(x,y) ;
...
int main(int argc, char **argv) {
  ...
  Image< float >* mask_ignore = NULL;
  if(args.name_orientation != "")
    orient = new Image<float>(args.name_orientation);
  if(args.name_mask != "")
    mask = new Image<float>(args.name_mask);
  if(args.name_mask_ignore != "")
    mask_ignore = new Image<float>(args.name_mask_ignore);

  vector< int > indexes(3);
  vector< float > micrometers(3);
  indexes[2] = 0;

  vector< double > cumPDF = getCumulativePdf(img, mask, mask_ignore);
