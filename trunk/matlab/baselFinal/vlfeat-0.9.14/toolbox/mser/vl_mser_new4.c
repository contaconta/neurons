            ...
            int nin, const mxArray *in[])
{
  ...
  if (dark_on_bright)
  {
    ...
    score            = vl_mser_get_variation   (filt) ; // modified
  ...
  if (bright_on_dark)
  {
    ...
    scoreinv       = vl_mser_get_variation   (filtinv) ; // modified
  ...
  if (nout > 1) {
    ...
    if (nout>2){
        odims [0]        = nregions + nregionsinv ;
        out [2] = mxCreateNumericArray(1, odims, mxDOUBLE_CLASS, mxREAL) ;
        pt               = mxGetPr(out [2]) ;
        ...
        for (i = nregions; i < nregions + nregionsinv; ++i)
            pt [i] = scoreinv [i-nregions] ; /* modified Inverted seed means dark on bright */
