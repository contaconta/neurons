...
vl_mser_process (VlMserFilt* f, vl_mser_pix const* im)
{
  ...
  vl_uint       *mer     = f-> mer ;
  float         *mervar  = f-> mervar;   // modified
  int            delta   = f-> delta ;
...
//       }
  ...
  */
    ...
    for(i = ner-1 ; i >= 0L  ; --i) {
      ...
      if (min_div < 1.0) {
        ...
        if((int) parent != i) {
          ...
          { 
              ++ ndup ; 
              if (er[i].variation < er[parent].variation) //modified: keep more stable regions
              {
                  er [parent] .max_stable = 0 ;
                  -- nmer ;
                  i = i+1;
                  continue;
              }
  ...
  if (f-> rmer < nmer) {
    ...
    f->rmer = nmer ;
    if (mervar)  vl_free(mervar) ;
    f->mervar  = mervar = vl_malloc( sizeof(float) * nmer*2) ;
  }
  ...
  for (i = 0 ; i < ner ; ++i) {
    if (er [i] .max_stable) {
        mervar [j] = er[i] .variation;
        mervar [j+nmer] = (float)(er[i] .area);
        mer [j++] = er [i] .index ;
    }
  }
}
