rocFolder = '/osshare/DropBox/Dropbox/aurelien/roc/heathrow/';
...
load([rocFolder 'hUs.mat']);
fpr{1}(1) = 0; tpr{1}(1) = 0;  tpr{1}(length(tpr{1}+1)) = tpr{1}(length(tpr{1})); fpr{1}( length(fpr{1}+1)) = 1;% corrections
fpr{3}(1) = 0; tpr{3}(1) = 0;  tpr{3}(length(tpr{3}+1)) = tpr{3}(length(tpr{3})); fpr{3}( length(fpr{3}+1)) = 1; % corrections
legnames{4} = 'Learned Combo';
legnames{3} = 'Standard Combo';
plot(fpr{1}, tpr{1}, 'o--', 'Color', Cols(2,:), 'LineWidth', .75); hold on;
plot(fpr{3}, tpr{3}, 'o-', 'Color', Cols(2,:), 'LineWidth', 2.5, 'MarkerFaceColor', Cols(2,:)); hold on;
