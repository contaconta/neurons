function rvc = ffEccentricitySomata(R, varargin)
...
for d = 1:length(R.D)
   ...
   if(isempty(R.Soma(d).Eccentricity))
     disp('encountered an empty value in ffEccentricitySomata');
     keyboard;
   end
   if(R.Soma(d).Time == 1)
     continue;
   ...
   nPoints = nPoints+1;
   rvc(nPoints) = R.Soma(d).Eccentricity;
end
