function rvc = ffTravelDistanceSomata(R, varargin)
...
for d = 1:length(R.D)
   ...
   if(isempty(R.Soma(d).TravelDistance))
     disp('encountered an empty value in ffTravelDistanceSomata');
     keyboard;
   end
   if(R.Soma(d).Time == 1)
     continue;
   ...
   nPoints = nPoints+1;
   rvc(nPoints) = R.Soma(d).TravelDistance;
end
