function rvc = ffdeltaPerimeterSomata(R, varargin)
...
for d = 1:length(R.D)
   ...
   if(isempty(R.Soma(d).deltaPerimeter))
     disp('encountered an empty value in ffdeltaPerimeterSomata');
     keyboard;
   end
   if(R.Soma(d).Time == 1)
     continue;
   ...
   nPoints = nPoints+1;
   rvc(nPoints) = R.Soma(d).deltaPerimeter;
end
