function vret =  fnMeanEndPoints(R, varargin)
...
for nTrack = 1:length(R.trkSeq)
    ...
    endpoints_tstep = zeros(1,length(trck));
    for nNT = 1:length(trck)
        ...
        endpoints = length(find(R.FILAMENTS(trck(nNT)).NumKids == 2));
        if isempty(endpoints)
            endpoints = 0;
        end
        endpoints_tstep(nNT) = endpoints;
    end
    nCount = nCount + 1;
    vret(nCount) = mean(endpoints_tstep);
end
