...
// 20110302
/**
   Function ComputeProbabilityMapShortestPath

   Input:
   - probability image:  (with values  between 0  and 1)

   - mask: same  size as image, the valid values are:
             - 0 the foreground pixels
             - 1 places where no path should cross
             - 2 source nodes

   Output:
   - Distance image. Each points represents the distance using Dijkstra shortest
     path to the closest point in the mask labeled with 2.
